...
void test1() {
    ...
    std::string path = "C:\\Users\\Vitya\\CLionProjects\\CPPExercises2021\\lesson13\\data\\" + caseName + "\\";
    std::string results = "C:\\Users\\Vitya\\CLionProjects\\CPPExercises2021\\lesson13\\resultsData\\" + caseName + "\\";
...
//        }
    ...
    std::vector<cv::Point2f> angelsin(4);
    std::vector<cv::Point2f> angelsout(4);

    angelsin[0]=cv::Point2f(0, 0);
    angelsin[1]=cv::Point2f(img1.cols, 0);
    angelsin[2]=cv::Point2f(0, img1.rows);
    angelsin[3]=cv::Point2f(img1.cols, img1.rows);

    angelsout.push_back(cv::Point2f(0, 0));
    angelsout.push_back(cv::Point2f(img0.cols, 0));
    angelsout.push_back(cv::Point2f(0, img0.rows));
    angelsout.push_back(cv::Point2f(img0.cols, img0.rows));

    perspectiveTransform(angelsin, angelsout, H10);
    cv::Rect br= boundingRect(angelsout);
...
void test3() {
    ...
    double median;
    {
        ...
        std::sort(distances.begin(), distances.end()); // GOOGLE: "cpp how to sort vector"
        std::cout << "matches10 distances min/median/max: " << distances[0] << "/" << distances[distances.size()/2] << "/" << distances[distances.size()-1] << std::endl;
        median = distances[distances.size()/3];
    }
    for (int k = 0; k < 2; ++k) {
        std::vector<cv::DMatch> matchesK;
        for (int i = 0; i < matches10.size(); ++i) {
...
//            rassert(match.trainIdx < points1.size(), 2347982739280182);
    ...
    std::vector<cv::Point2f> anglesin(4);
    std::vector<cv::Point2f> anglesout(4);

    anglesin[0]=cv::Point2f(0, 0);
    anglesin[1]=cv::Point2f(img1.cols, 0);
    anglesin[2]=cv::Point2f(0, img1.rows);
    anglesin[3]=cv::Point2f(img1.cols, img1.rows);

    perspectiveTransform(anglesin, anglesout, H10);

    anglesout.push_back(cv::Point2f(0, 0));
    anglesout.push_back(cv::Point2f(img0.cols, 0));
    anglesout.push_back(cv::Point2f(0, img0.rows));
    anglesout.push_back(cv::Point2f(img0.cols, img0.rows));


    cv::Rect br= boundingRect(anglesout);
